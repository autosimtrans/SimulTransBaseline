...
class DecoderLayer(Layer):
    ...
    def forward(self,
        ...
        if len(enc_outputs) == 1:
            '''
            print('self:', self.preprocesser2(self_attn_output).shape)
            print('enc:', enc_outputs[0].shape)
            print('cross:', cross_attn_bias.shape)
            print('k:', cache['k'].shape)
            print('v:', cache['v'].shape)
            if 'static_k' in cache:
                print('sk:', cache['static_k'].shape)
                print('sv:', cache['static_v'].shape)
            '''
            cross_attn_output = self.cross_attn(
                self.preprocesser2(self_attn_output), enc_outputs[0], enc_outputs[0],
                cross_attn_bias)
                # cross_attn_bias, cache)
        else:
            qs = self.preprocesser2(self_attn_output)
            cross_attn_outputs = []
            for i in range(qs.shape[1]):
                #q = layers.unsqueeze(input=qs[:, i, :], axes=[1])
                q = qs[:, i:i+1, :]
                if i >= len(enc_outputs):
                    e = enc_outputs[-1]
                else:
                    e = enc_outputs[i]
                cross_attn_outputs.append(self.cross_attn(
                    q, e, e, cross_attn_bias[:, :, i:i+1, :e.shape[1]], cache))
            cross_attn_output = layers.concat(cross_attn_outputs, axis=1)
            # embed()
...
class Decoder(Layer):
    ...
    def forward(self,
        ...
        for i, decoder_layer in enumerate(self.decoder_layers):
            dec_output = decoder_layer(dec_input, enc_outputs, self_attn_bias,
                                       cross_attn_bias,
...
class WrapDecoder(Layer):
    ...
    def forward(self,
        ...
        dec_input = layers.dropout(emb,
                                   dropout_prob=self.emb_dropout,
                                   is_test=False) if self.emb_dropout else emb
        dec_output = self.decoder(dec_input, enc_outputs, trg_slf_attn_bias,
...
class Transformer(Layer):
    ...
    def forward(self, src_word, src_pos, src_slf_attn_bias, trg_word, trg_pos,
        ...
        src_len = src_word.shape[1]
        if self.waitk >= src_len or self.waitk == -1:
            enc_outputs = [self.encoder(src_word, src_pos, src_slf_attn_bias)]
    ...
    def _beam_search(self,
        ...
        # run encoder
        # enc_output = self.encoder(src_word, src_pos, src_slf_attn_bias)
        if waitk < 0 or waitk > src_word.shape[1]:
            # enc_output = self.encoder(src_word, src_pos, src_slf_attn_bias)
            enc_outputs = [self.encoder(src_word, src_pos, src_slf_attn_bias)]
        else:
            enc_outputs = []
            for i in range(waitk, src_word.shape[1]+1):
                enc_output = self.encoder(src_word[:, :i], src_pos[:, :i], src_slf_attn_bias[:, :, :i, :i])
                enc_outputs.append(enc_output)
    ...
    def _greedy_search(self,
        ...
        '''
        enc_outputs = []
        for i in range(1, max_src_len+1):
            _src_word = layers.concat(src_word_dict[i], axis=0)
            _src_pos = layers.concat(src_pos_dict[i], axis=0)
            _src_slf_attn_bias = layers.concat(src_attn_dict[i], axis=0)
            # enc_output = self.encoder(_src_word, _src_pos, _src_slf_attn_bias)
            enc_output = self.encoder(_src_word[:, :i], _src_pos[:, :i], _src_slf_attn_bias[:, :, :i, :i])
            # embed()
            enc_outputs.append(enc_output)
        '''
        ...
        for i in range(max_len):
            ...
            for j, l in enumerate(src_lens):
                if l > i + waitk:
                    # mask eos
                    mask[j][eos_id] = -1e9
            scores += fluid.core_avx.VarBase(mask.astype("float32"))
            '''
