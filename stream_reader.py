...
def prepare_infer_stream_input(insts_list, src_pad_idx, bos_idx, n_head, place, src_max_len):
    """
    Put all padded data needed by beam search decoder into a list.
    """
    data_inputs_list = []
    for insts in insts_list:
        for i in range(len(insts)-1):
            insts[i] = [insts[i][0][:-1]]
        src_word, src_pos, src_slf_attn_bias, _src_max_len = pad_batch_data(
            [inst[0] for inst in insts], src_pad_idx, n_head, is_target=False, max_len=src_max_len)
        # start tokens
        trg_word = np.asarray([[bos_idx]] * len(insts), dtype="int64")
        trg_src_attn_bias = np.tile(src_slf_attn_bias[:, :, ::src_max_len, :],
                                    [1, 1, 1, 1]).astype("float32")
...
class DataProcessor(object):
    ...
    :type seed: int
    :param stream: whether the input is stream.
    :type stream: bool
    """
    def __init__(self,
                 src_vocab_fpath,
        ...
        if self._stream:
            ...
            for src_list, read_list in zip(filter_src, real_read):
                ...
                for line, read_num in zip(src_list, read_list):
                    ...
                    self._sample_infos[-1].append(SampleInfo(i, max(lens), min(lens)))
                    i += 1
        else:
    ...
    def batch_generator(self, batch_size, use_token_batch):
        def __impl__():
            ...
            if self._stream:
                batches.append([])
                for info in infos:
                    batch_creator = SentenceBatchCreator(len(info))
            ...
            for batch in batches:
                if self._stream:
                    batch_ids_list = [[info.i for info in _batch] for _batch in batch]
                else:
                ...
                if self._stream:
                    ...
                    for step in range(max_src_len):
                        ...
                        for i in range(len(data_inputs)):
                            ...
                            (src_word, src_pos, src_slf_attn_bias, trg_word, cross_attn_bias) = data_inputs[i]
                            if step+1 >= len(src_word):
                                _src_word.append(src_word[-1])
                                _src_pos.append(src_pos[-1])
                                _src_attn.append(src_slf_attn_bias[-1])
                                _cross_attn.append(cross_attn_bias[-1])
                            else:
                                _src_word.append(src_word[step])
                                _src_pos.append(src_pos[step])
                                _src_attn.append(src_slf_attn_bias[step])
                                _cross_attn.append(cross_attn_bias[step])

                        src_word_list.append(_src_word)
                        src_pos_list.append(_src_pos)
